...
def knowledge_distillation_loss(y_true, y_pred):
    ...
    # First 10 are predicted probabilities of the ensemble(uses temperature)
    # Last 10 are the onehot true labels
    y_true_ensemble = y_true[:, 10:]
    ...
    loss1 = keras.losses.categorical_crossentropy(y_true_ensemble, y_pred_temperature)
...
class DistilledModel(object):
    ...
    def train(self, sess, x_train, y_train, batch_size, num_epochs):
        print("\nTraining distilled model:")
        with sess.as_default():
            # Convert predictions of ensemble and true one-hot y_train to a single array
            y_train_distilled = self.ensemble_model.predict(x_train).eval()
